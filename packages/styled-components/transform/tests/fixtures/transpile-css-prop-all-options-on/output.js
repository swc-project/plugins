...
const SpreadObjectPropMixedInputs = (p) => {
  ...
  return (
    <_StyledP13
      ...
      $_css9={getAfterValue()}
      $_css10={globalVar}
      $_css11={getAfterValue()}
      $_css12={p.background}
      $_css13={globalVar}
      $_css14={getAfterValue()}
    >
      A
    </_StyledP13>
  );
...
/* insert before usage for non-local scope styled HOC targets */ const ImportedComponentUsage =
  (p) => <_StyledSomeComponent />;
const RequiredComponentUsage = (p) => <_StyledSomeOtherComponent />;
const ObjectInterpolation = (p) => {
  const theme = useTheme();
  return <_StyledP14 $_css15={theme.colors.red}>H</_StyledP14>;
};
const ObjectInterpolationCustomComponent = (p) => {
  const theme = useTheme();
  return <_StyledThing3 $_css16={theme.colors.red}>H</_StyledThing3>;
};
const ObjectInterpolationInKey = (p) => {
  const theme = useTheme();
  return <_StyledThing4 $_css17={theme.breakpoints.md}>H</_StyledThing4>;
};
const ObjectFnInterpolationInKey = (p) => {
  const theme = useTheme();
  return <_StyledThing5 $_css18={theme.breakpoints.md()}>H</_StyledThing5>;
};
const ObjectFnSimpleInterpolationInKey = (p) => {
  const foo = "@media screen and (max-width: 600px)";
  return <_StyledThing6 $_css19={foo}>H</_StyledThing6>;
};
